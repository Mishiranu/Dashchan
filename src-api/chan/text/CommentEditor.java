...
public class CommentEditor
{
			...
			if (c == '\n')
			{
				...
				if (lineEnd > lineStart)
				{
					...
					if (mayCutStart || mayCutEnd)
					{
						...
						{
							if (cutStart) lineStart += openLength;
							if (cutEnd) lineEnd -= closeLength;
						}
					}
	...
	public String removeTags(String comment)
	{
		if (comment != null)
		{
			...
			for (int i = 0; i < tags.size(); i++)
			{
				...
				if (tag != null)
				{
					if (tag.open != null) comment = comment.replace(tag.open, "");
					if (tag.close != null) comment = comment.replace(tag.close, "");
				}
			}
		}
		return comment;
	...
	public static class WakabaMarkCommentEditor extends CommentEditor
	{
		...
		public String getTag(int what, boolean close, int length)
		{
			if (what == ChanMarkup.TAG_STRIKE && mSerialStrike)
			{
				if (close)
				{
					...
					for (int i = 0; i < length; i++) builder.append("^H");
		...
		public FormatResult formatSelectedText(Editable editable, int what, int start, int end)
		{
			if (what == ChanMarkup.TAG_STRIKE && mSerialStrike)
			{
				...
				else
				{
					...
					if (count > 0)
					{
						for (int i = 0; i < count; i++) builder.append("^H");
		...
		public String removeTags(String comment)
		{
			...
			if (comment != null) comment = comment.replace("^H", "");
			return comment;
		}
	}
}