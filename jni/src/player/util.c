...
void queueClear(Queue * queue, void callback(void * data))
{
	...
	{
		if (callback != NULL && queueItem->data != NULL) callback(queueItem->data);
...
void queueAdd(Queue * queue, void * data)
{
	...
	queueItem->data = data;
	queueItem->next = NULL;
	if (queue->last == NULL) queue->first = queueItem; else queue->last->next = queueItem;
...
void * blockingQueueGet(BlockingQueue * blockingQueue, int wait)
{
	...
	while (!blockingQueue->interrupted)
	{
		...
		else if (wait) pthread_cond_wait(&blockingQueue->cond, &blockingQueue->mutex);
		else break;
	}
	pthread_mutex_unlock(&blockingQueue->mutex);
	return data;
...
void bufferQueueClear(BufferQueue * bufferQueue, void callback(BufferItem * bufferItem))
{
	while (1)
	{
		...
		if (bufferItem == NULL) break;
		if (callback != NULL) callback(bufferItem);
		queueAdd(&bufferQueue->freeQueue, bufferItem);
	}
}
...
{
	if (bufferSize > bufferQueue->bufferSize) bufferQueue->bufferSize = bufferSize;
}
...
void sparceArrayDestroy(SparceArray * sparceArray, void callback(void * data))
{
	if (callback != NULL)
	{
		for (int i = 0; i < sparceArray->count; i++)
		{
			...
			if (sparceArrayItem.data != NULL) callback(sparceArrayItem.data);
		}
	}
