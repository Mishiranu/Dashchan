...
jlong init(JNIEnv * env, jstring fileName)
{
	...
	if (status == D_GIF_SUCCEEDED)
	{
		...
		{
			status = file->Error;
			if (status == D_GIF_SUCCEEDED) status = D_GIF_ERR_CUSTOM;
		}
		else if (file->ImageCount == 0) status = D_GIF_ERR_CUSTOM; else status = D_GIF_SUCCEEDED;
	}
	if (status == D_GIF_SUCCEEDED)
	{
		int when = 0;
		...
		for (int i = 0; i < file->ImageCount; i++)
		{
			...
			for (int j = 0; j < image->ExtensionBlockCount; j++)
			{
				...
				if (extension->Function == GRAPHICS_EXT_FUNC_CODE && extension->ByteCount == 4)
				{
					...
					if (flags & 1) data->transparentIndex = extension->Bytes[3] & 0xff;
...
void destroy(JNIEnv * env, jlong pointer)
{
	...
	int error;
	if (decoder->file != NULL) DGifCloseFile(decoder->file, &error);
	if (decoder->datas != NULL) free(decoder->datas);
	if (decoder->previousColors != NULL) free(decoder->previousColors);
	free(decoder);
}
...
static void drawImage(Decoder * decoder, int index, int * colors)
{
	...
	if (index > 0)
	{
		...
		else if (data->disposalMethod == 3) memcpy(colors, decoder->previousColors, 4 * width * height);
	}
	...
	if (data->disposalMethod == 3)
	{
		...
		{
			if (decoder->previousColors == NULL) decoder->previousColors = malloc(4 * width * height);
			memcpy(decoder->previousColors, colors, 4 * width * height);
			decoder->hasPrevious = 1;
		}
	}
	else decoder->hasPrevious = 0;
	ColorMapObject * colorMap = image->ImageDesc.ColorMap;
	if (colorMap == NULL) colorMap = decoder->file->SColorMap;
...
jint draw(JNIEnv * env, jlong pointer, jobject bitmap)
{
	...
	if (decoder->duration > 0)
	{
		...
		if (decoder->startTime == 0) decoder->startTime = time;
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (position >= nextData->startTime) index = i; else break;
	}
	if (count > 1) delay = (index + 1 < count ? decoder->datas[index + 1].startTime : decoder->duration) - position;
	if (decoder->lastIndex != index)
	{
		...
		if (colors != NULL)
		{
			...
			{
				for (int i = decoder->lastIndex + 1; i <= index; i++) drawImage(decoder, i, colors);
			}
			else
			{
				decoder->hasPrevious = 0;
				memset(colors, 0, 4 * decoder->file->SWidth * decoder->file->SHeight);
				for (int i = 0; i <= index; i++) drawImage(decoder, i, colors);
			}
