...
public class PullableWrapper implements AbsListView.OnScrollListener
{
	...
	private static class IcsProgress implements PullPainter
	{
		...
		public void setState(State state, int padding)
		{
			if (mState != state)
			{
				...
				switch (mState)
				{
					...
					{
						mTimeIdleStart = time;
						if (mPreviousState == State.LOADING) mTimeLoadingToIdleStart = time;
		...
		public int getAndResetIdlePullStrain()
		{
			if (mStartIdlePullStrain == 0) return 0;
			try
			{
		...
		public void draw(Canvas canvas, int padding, float density)
		{
			...
			if (state == State.LOADING || mTimeLoadingToIdleStart > 0L)
			{
				...
				if (startTransient) elapsed += cycle;
	...
	private static class LollipopProgress implements PullPainter
	{
				...
				switch (mState)
				{
					...
					case LOADING:
					{
						...
						if (!loadingToLoading)
						{
							...
							if (mPreviousState == State.IDLE) mSpinOffset = 0f;
						}
						break;
					}
				}
		...
		public int getAndResetIdlePullStrain()
		{
			if (mStartFoldingPullStrain == 0) return 0;
			try
			{
		...
		public void draw(Canvas canvas, int padding, float density)
		{
			...
			if (value != 0f)
			{
				...
				else
				{
					...
					if (value >= 1f) mSpinOffset = (value - 1f) * 0.5f;
				}
