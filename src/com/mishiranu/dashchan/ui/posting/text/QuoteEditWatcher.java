...
public class QuoteEditWatcher implements TextWatcher
{
	...
	private boolean contains(CharSequence s, int start, int end, char what)
	{
		...
		{
			if (s.charAt(i) == what) return true;
		}
		return false;
	}
	...
	public void afterTextChanged(Editable s)
	{
		...
		if (mUpdateQuotes)
		{
			for (QuoteSpan quoteSpan : s.getSpans(0, s.length(), QuoteSpan.class)) s.removeSpan(quoteSpan);
			if (text == null) text = s.toString();
			int index = 0;
			while (true)
			{
				...
				if (start > 0 && text.charAt(start - 1) != '\n') index++; else
				{
					...
					if (finish) end = text.length(); else index = end;
		...
		if (mUpdateLinks)
		{
			for (LinkSpan linkSpan : s.getSpans(0, s.length(), LinkSpan.class)) s.removeSpan(linkSpan);
			if (text == null) text = s.toString();
			int index = 0;
			while (true)
			{
				...
				{
					if (isNumber(text.charAt(i))) numbers++; else break;
				}
				if (numbers == 0) index++; else
				{
					if (start == 0 || text.charAt(start - 1) == '\n')
					{
						// Remove quote span in the start of line
						for (QuoteSpan quoteSpan : s.getSpans(start, start, QuoteSpan.class)) s.removeSpan(quoteSpan);
					}
