...
public class Preferences
{
	...
	private static boolean isNetworkAvailable(String value, String defaultValue)
	{
		...
		{
			case GENERIC_VALUE_NETWORK_ALWAYS: return true;
			case GENERIC_VALUE_NETWORK_WIFI_3G: return NetworkObserver.getInstance().isMobile3GConnected();
			case GENERIC_VALUE_NETWORK_WIFI: return NetworkObserver.getInstance().isWifiConnected();
			case GENERIC_VALUE_NETWORK_NEVER: return false;
		}
	...
	public static int getAutoRefreshMode()
	{
		...
		if (VALUE_AUTO_REFRESH_MODE_SEPARATE.equals(value)) return AUTO_REFRESH_MODE_SEPARATE;
		if (VALUE_AUTO_REFRESH_MODE_DISABLED.equals(value)) return AUTO_REFRESH_MODE_DISABLED;
		if (VALUE_AUTO_REFRESH_MODE_ENABLED.equals(value)) return AUTO_REFRESH_MODE_ENABLED;
	...
	public static String getCaptchaTypeForChanConfiguration(String chanName)
	{
		...
		if (captchaTypeValue != null && captchaTypeValue.startsWith(VALUE_CAPTCHA_START))
		{
			...
			{
				if (supportedCaptchaType.equals(captchaType)) return captchaType;
			}
		}
	...
	public static ArrayList<String> getChansOrder()
	{
		...
		if (data != null)
		{
			try
			{
				...
				for (int i = 0; i < jsonArray.length(); i++) chanNames.add(jsonArray.getString(i));
				return chanNames;
			}
			catch (JSONException e)
			{
	...
	public static void setChansOrder(List<String> chanNames)
	{
		...
		for (String chanName : chanNames) jsonArray.put(chanName);
			...
			String threadNumber, String threadTitle)
	{
		...
		for (int i = builder.length() - 2; i >= 0; i--)
		{
			...
			if (c == '\\')
			{
				...
				if (replaceTo != null) builder.replace(i, i + 2, replaceTo);
			}
		}
	...
	public static File getDownloadDirectory()
	{
		...
		if (pathSegments.size() > 0)
		{
			...
			if (first.exists() && first.isDirectory()) absolute = true;
		}
		if (!absolute) dir = new File(Environment.getExternalStorageDirectory(), path);
		dir.mkdirs();
		return dir;
	}
	...
	public static boolean isDownloadSubdir(boolean multiple)
	{
		...
		if (VALUE_DOWNLOAD_SUBDIR_DISABLED.equals(value)) return false;
		if (VALUE_DOWNLOAD_SUBDIR_MULTIPLE.equals(value)) return multiple;
		if (VALUE_DOWNLOAD_SUBDIR_ENABLED.equals(value)) return true;
		return false;
	}
	...
	public static int getFavoritesOrder()
	{
		...
		if (VALUE_FAVORITES_ORDER_DATE_DESC.equals(value)) return FAVORITES_ORDER_ADD_TO_THE_TOP;
		if (VALUE_FAVORITES_ORDER_DATE_ASC.equals(value)) return FAVORITES_ORDER_ADD_TO_THE_BOTTOM;
		if (VALUE_FAVORITES_ORDER_TITLE.equals(value)) return FAVORITES_ORDER_BY_TITLE;
	...
	public static int getPagesListMode()
	{
		...
		if (VALUE_PAGES_LIST_PAGES_FIRST.equals(value)) return PAGES_LIST_MODE_PAGES_FIRST;
		if (VALUE_PAGES_LIST_FAVORITES_FIRST.equals(value)) return PAGES_LIST_MODE_FAVORITES_FIRST;
		if (VALUE_PAGES_LIST_HIDE_PAGES.equals(value)) return PAGES_LIST_MODE_HIDE_PAGES;
	...
	private static String generatePassword()
	{
		...
		for (int i = 0, count = 10 + random.nextInt(6); i < count; i++)
		{
			...
			if (value < 26) value = 0x41 + value;
			else if (value < 26 + 26) value = 0x61 + value - 26;
			else value = 0x30 + value - 26 - 26;
	...
	public static int getThemeResource()
	{
		...
		{
			if (VALUES_THEME[i].equals(theme)) return VALUES_THEME_IDS[i];
		}
	...
	public static int getVideoCompletionMode()
	{
		...
		if (VALUE_VIDEO_COMPLETION_NOTHING.equals(value)) return VIDEO_COMPLETION_MODE_NOTHING;
		if (VALUE_VIDEO_COMPLETION_LOOP.equals(value)) return VIDEO_COMPLETION_MODE_LOOP;
