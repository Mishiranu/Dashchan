...
public class VideoPlayer
{
	...
	private static class PlayerTextureView extends TextureView implements TextureView.SurfaceTextureListener
	{
		...
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
		{
			...
			if (dimensions != null && dimensions.x > 0 && dimensions.y > 0)
			{
				...
				if (dimensions.x * height > dimensions.y * width) height = dimensions.y * width / dimensions.x;
				else width = dimensions.x * height / dimensions.y;
		...
		synchronized (this)
		{
			if (!mConsumed && mInitialized && mPlaying != playing)
			{
				...
				if (playing)
				{
					...
					if (seekToPosition >= 0L) setPosition(seekToPosition);
				}
				else
				{
	...
	public void free()
	{
		synchronized (this)
		{
			...
			{
				mConsumed = true;
				if (mInputHolder != null) mInputHolder.close();
				cancelSetPosition();
				synchronized (mSeekerThread)
				{
					mSeekerThread.notifyAll();
	...
	private static class NativeBridge
	{
		...
		public int onRead(int size)
		{
			...
			{
				if (player.mInputHolder == null || player.mCancelNativeRequests) return -1;
		...
		public long onSeek(long position, int whence)
		{
			...
			{
				if (player.mInputHolder == null || player.mCancelNativeRequests) return -1;
				if (whence == 0x10000) return player.mInputHolder.getSize(); // AVSEEK_SIZE == 0x10000
				try
				{
		...
		public void onMessage(int what)
		{
			...
			if (player != null)
			{
				switch (what)
				{
					case 1: // BRIDGE_MESSAGE_PLAYBACK_COMPLETE
					{
						HANDLER.sendMessageDelayed(HANDLER.obtainMessage(MESSAGE_PLAYBACK_COMPLETE, player), 200);
						break;
					}
					case 2: // BRIDGE_MESSAGE_SIZE_CHANGED
					{
						HANDLER.obtainMessage(MESSAGE_SIZE_CHANGED, player).sendToTarget();
						break;
					}
					case 3: // BRIDGE_MESSAGE_START_SEEKING
					{
						HANDLER.removeMessages(MESSAGE_END_SEEKING);
						if (player.mLastBuffering) HANDLER.obtainMessage(MESSAGE_START_SEEKING, player).sendToTarget();
						else HANDLER.sendMessageDelayed(HANDLER.obtainMessage(MESSAGE_START_SEEKING, player), 500);
						break;
					}
					case 4: // BRIDGE_MESSAGE_END_SEEKING
					{
